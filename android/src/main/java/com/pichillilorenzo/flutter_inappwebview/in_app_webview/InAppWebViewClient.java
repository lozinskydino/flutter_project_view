...
//    channel.invokeMethod("onLoadError", obj);
  ...
  private WebResourceResponse makeRequest(String url, Map<String, String> headers, String reffer) {
    ...
    try {
      ...
      //DnsOverHttps customDns = new DnsOverHttps.Builder().client(bootstrapClient)
      ...
      OkHttpClient httpClient = bootstrapClient.newBuilder()
      ...
      if (needRedirect(code)) {
        ...
        if (!(location.startsWith("http://") || location.startsWith("https://"))) {
          ...
          return makeRequest(location, headers, url);
        }
      ...
      System.out.println("httpConnection::ctype::" + contentType);
  ...
  private WebResourceResponse okHttpResponseToWebResourceResponse(Response resp) {
    ...
    if (contentTypeValue != null) {
      if (contentTypeValue.indexOf("charset=") > 0) {
        ...
        return new WebResourceResponse(contentType, charset, inStream);
      } else {
        return new WebResourceResponse(contentTypeValue, null, inStream);
      }
    } else {
  ...
  /**
   * verifica se tem cookie nas headers e deixa passar direto
   * @param headers
   */
  ...
  /*class CustomDns implements Dns {
    ...
    public List<InetAddress> lookup(String hostname) {
      try {
        return InetAddress.getAllByName("<YOUR_IP>").toList();
      } catch (Exception e) {
        throw UnknownHostException("Failed dns lookup of $hostname");
      }
    }
  }*/
